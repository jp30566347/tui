...
# MACRO_TUI_VERSION to pin a tag such as macro-tui-v0.1.0.
#
...
# macro-tui-vX.Y.Z. "latest" across the repo could be the other one, so the
# newest macro-tui tag is looked up instead of assuming it.
if [ "$VERSION" = "latest" ]; then
    VERSION=$(fetch_stdout "https://api.github.com/repos/$REPO/releases?per_page=30" |
        tr ',' '\n' | grep '"tag_name"' | cut -d'"' -f4 |
        grep "^macro-tui-v" | head -1)
    [ -n "$VERSION" ] || err "could not find a published macro-tui release"
fi
